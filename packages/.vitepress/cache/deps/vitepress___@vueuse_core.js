...
function useElementSize(target, initialSize = { width: 0, height: 0 }, options = {}) {
  ...
  const { stop: stop1 } = useResizeObserver(
    ...
    ([entry]) => {
      const boxSize =
        box === 'border-box' ? entry.borderBoxSize : box === 'content-box' ? entry.contentBoxSize : entry.devicePixelContentBoxSize;
...
function useFocus(target, options = {}) {
  ...
  const focused = computed({
    ...
    set(value) {
      ...
      else if (value && !innerFocused.value) (_b = targetElement.value) == null ? void 0 : _b.focus();
    },
  });
  watch(
    targetElement,
...
function useNow(options = {}) {
  ...
  const controls =
    interval === 'requestAnimationFrame' ? useRafFn(update, { immediate: true }) : useIntervalFn(update, interval, { immediate: true });
  if (exposeControls) {
    return {
      now: now2,
      ...controls,
...
function useTextDirection(options = {}) {
  ...
  function getValue2() {
    var _a, _b;
    return (_b = (_a = document2 == null ? void 0 : document2.querySelector(selector)) == null ? void 0 : _a.getAttribute('dir')) != null
      ? _b
      : initialValue;
  }
